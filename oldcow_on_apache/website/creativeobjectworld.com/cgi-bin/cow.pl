...
my $browsr=$ENV{'HTTP_USER_AGENT'};

#if($browsr =~/google/i){
#	$browsr	= 'google spider';
#}
...
my $content = &get_url($this_ip.$script.$params."&ip=$ip&br=$br"); # call the page and get the results..
...
sub read_source { # read te source index file that contains current personal cow server IP
  my $filename = '../index.html'; # where the IP of the current personal server is found..
...
sub get_url { # returns a web page from the passed URL..
  ...
  if($remote){
    $remote->autoflush(1);
    print $remote ("GET /$document HTTP/1.0\nHost: $host\nUser-Agent: $browsr\015\012\015\012");
    my @lines = <$remote>;
    close $remote;
    my $c=0;
