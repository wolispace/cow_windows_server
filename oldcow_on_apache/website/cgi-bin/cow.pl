...
if($offline) {
  $db_driver  ='mysql:wolispac_cow:127.0.0.1';
  $db_user    ='boris';
  $db_pw      ='abc678';
  $cow        ='cow2.pl';
...
$rel_words='at|as|to|on|in|near|far from|far away from|away from|under|between|above|around|encompassing|beside|behind|leaning against|next to|through|against|with|by|over|across|facing|leaning|looking|leading|heading|pointing|going|running|to the|north|east|west|south|up|down|from|off';
...
# spelling not needed thanks to firefox
...
#: combine
      ...
      if($op1 =~ /(.+) (of|made of) (.+)/i){ $op1 = $1; $ma1 = $3;  }                     # combine "a jelly fish" made of _fish_ made by "fisherman" with "some pastry" made of _pastry_ by "baker" in $loc to make "fish pie";
      if($op2 =~ /(.+) (of|made of) (.+)/i){ $op2 = $1; $ma2 = $3;  }
      ...
      if($newqty>0){
        ...
        foreach my $key (@keys){
          if($keep1>0){
            ...
            &db_update_records('objects',"loc=0,worth=0,switch=0,code=''",'obj_id='.$key); # delete this.. send it to the void..
