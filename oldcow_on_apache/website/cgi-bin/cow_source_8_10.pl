...
#: new
      ...
      # find any objects with matching calss and name in this loc and increate its number dont make new obj
      ($target,$found_qty) = &db_find_first('objects','obj_id,qty',"loc=$loc and pwd='' and (class='$class' or pclass='$class') and name='$name'");
      if($target>0){
        ...
        &db_update_records('objects',"qty=$found_qty","obj_id=$target");
      }else{
