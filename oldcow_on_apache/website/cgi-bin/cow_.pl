...
my $op3=&find_field($4);
my $op4=$5;
if($op1=~/(.+) (from|by|\w+ by) (.+)/i){ $op1=$1; $sp1=$3; }if($op2=~/(.+) (from|by|\w+ by) (.+)/i){ $op2=$1; $sp2=$3; }($qty1,$size1,$qty_type1,$attribs1,$material1,$class1,$name1,$extra1,$worth1,$code1,$colour1)=&parse_obj(&find_field($op1));
...
&db_update_records('objects',"loc=0,worth=0,swicth=0,code=''",'obj_id='.$key);
}}my @keys=keys %found2;
foreach my $key (@keys){
if($keep2>0){
if($found2{$key}>=$keep2){
